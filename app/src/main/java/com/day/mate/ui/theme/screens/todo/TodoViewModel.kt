...
// كلاس الأحداث لإرسال التنبيهات للواجهة
...
class TodoViewModel(private val repository: TodoRepository) : ViewModel() {
    ...
    // تعريف الـ Event Flow لاستقبال الرسائل في الـ UI
    ...
    // Formatter موحد لتجنب مشاكل الوقت
    ...
    fun loadTaskById(id: Int) {
        viewModelScope.launch {
            ...
            if (taskToEdit != null) {
                ...
                // تحسين قراءة الوقت لتجنب الكراش
                _time.value = try {
                    LocalTime.parse(taskToEdit.time, timeFormatter)
                } catch (e: Exception) {
                    try {
    ...
    fun createTask() {
        ...
        // 🔥 التحقق من التكرار (نفس الاسم ونفس التاريخ)
        val isDuplicate = _todos.value.any {
            it.title.equals(titleValue, ignoreCase = true) &&
                    it.date == _date.value.toString()
        }

        if (isDuplicate) {
            viewModelScope.launch {
                // 🔥 الرسالة اللي هتظهر لليوزر
                _uiEvent.emit(UiEvent.ShowSnackbar("يوجد مهمة بنفس الاسم في هذا اليوم!"))
            }
            return // وقف العملية وماتحفظش
        }
        ...
        viewModelScope.launch {
            val newTodo = Todo(
                ...
                time = _time.value.format(timeFormatter),
            ...
            _uiEvent.emit(UiEvent.ShowSnackbar("تمت الإضافة بنجاح"))
    ...
    fun updateTask(id: Int) {
        ...
        viewModelScope.launch {
            ...
            _uiEvent.emit(UiEvent.ShowSnackbar("تم التعديل بنجاح"))
        }
    }

    private suspend fun addTodoToFirestore(todo: Todo): String {
        val userId = auth.currentUser?.uid ?: return ""
        val docRef = db.collection("users").document(userId).collection("tasks").document()
    ...
    private fun updateTodoInFirestore(todo: Todo) {
        ...
        db.collection("users").document(userId).collection("tasks").document(rId).set(taskMap)
    }
    ...
    private fun deleteTodoFromFirestore(todo: Todo) {
        ...
        val rId = todo.remoteId
        if (rId.isNullOrEmpty()) return
        db.collection("users").document(userId).collection("tasks").document(rId).delete()
    }
    ...
    private fun updateDoneStateInFirestore(todo: Todo) {
        ...
        val rId = todo.remoteId
        if (rId.isNullOrEmpty()) return
        db.collection("users").document(userId).collection("tasks").document(rId).update("isDone", todo.isDone)
    }
